...
public class LibreriaGraphic extends JPanel implements Command {
    ...
    @Override
    public void execute(Libreria libreria){
    ...
    private void reset(Libreria libreria, Object[][] data, String[] columnNames){
        ArrayList<Libro> libri = libreria.getLibri();
    ...
    private void add(Libreria libreria, Object[][] data, String[] columnNames){
        ...
        if(nuovoTitolo!=null && !nuovoTitolo.isEmpty()){
            if(nuovoAutore!=null && !nuovoAutore.isEmpty()){
                nuovoLibro.setTitolo(nuovoTitolo);
                nuovoLibro.setAutore(nuovoAutore);
                nuovoLibro.setISBN(new Random().nextInt(1000000000,1234567899));
            }
        }
        else{
    ...
    private void salvaSuDesktop(Libreria libreria){
        if(libreria.salva()!=null)
            JOptionPane.showMessageDialog(null,"La tua libreria è stata salvata sul desktop!");
        else
            JOptionPane.showMessageDialog(null,"Errore! Non è stato possibile salvare la libreria");
    }
